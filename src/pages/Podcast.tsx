...
export const Podcast = () => {
  //   const [detail, setDetail] = useState<typeof mockDetail>();
  //   const [episodes, setEpisodes] = useState<(typeof mockEpisode)[]>([]);

  //   useEffect(() => {
  //     fetch(
  //       `https://api.allorigins.win/get?url=${encodeURIComponent('https://itunes.apple.com/lookup?id=1535809341&media=podcast&entity=podcastEpisode&limit=20')}`
  //     )
  ...
  const { detail, episodes } = useLoaderData();
      ...
      <div className={styles.rightPanel}>
        {detail ? (
          <ul className={styles.list}>
            {episodes?.map((item: typeof mockData) => (
              <li key={item.trackId}>
